import os, yaml, argparse

if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Load YAML config file')
    parser.add_argument('--cfg_path', type=str, required=True, help='Path to the YAML configuration file')
    args = parser.parse_args()

    with open(args.cfg_path, 'r') as file:
        cfg = yaml.safe_load(file)

    llms = cfg.get('llms', [])
    suffix = cfg.get('suffix', '')
    attack_tool_types = cfg.get('attack_tool', [])
    write_db = cfg.get('write_db', False)
    read_db = cfg.get('read_db', False)
    defense_type = cfg.get('defense_type', None)
    injection_method_cfg = cfg['injection_method']  # 'direct_prompt_injection', 'memory_attack', 'observation_prompt_injection', 'clean'
    attack_types = cfg.get('attack_types', [])
    attacker_tools_override = cfg.get('attacker_tools_path')
    tasks_path_override = cfg.get('tasks_path')
    task_num = cfg.get('task_num')

    if isinstance(injection_method_cfg, list):
        injection_methods = injection_method_cfg
    else:
        injection_methods = [injection_method_cfg]

    suffix_str = f'_{suffix}' if suffix else ''

    for injection_method in injection_methods:
        for attack_tool_type in attack_tool_types:
            for llm in llms:
                for attack_type in attack_types:
                    if llm.startswith('gpt') or llm.startswith('gemini') or llm.startswith('claude'):
                        llm_name = llm
                        backend=None
                    elif llm.startswith('ollama'):
                        llm_name = llm.split('/')[-1]
                        backend='ollama'
                    else:
                        llm_name = llm
                        backend=None

                    log_path = f'logs/{injection_method}/{llm_name}'
                    database = None
                    if injection_method != 'clean':
                        database = f'memory_db/direct_prompt_injection/{attack_type}_gpt-4o-mini'

                    tasks_path_arg = tasks_path_override

                    if attack_tool_type == 'all':
                        attacker_tools_path = attacker_tools_override or 'data/all_attack_tools.jsonl'
                    elif attack_tool_type == 'non-agg':
                        attacker_tools_path = attacker_tools_override or 'data/all_attack_tools_non_aggressive.jsonl'
                    elif attack_tool_type == 'agg':
                        attacker_tools_path = attacker_tools_override or 'data/all_attack_tools_aggressive.jsonl'
                    elif attack_tool_type == 'test':
                        attacker_tools_path = attacker_tools_override or 'data/attack_tools_test.jsonl'
                        if tasks_path_arg is None:
                            tasks_path_arg = 'data/agent_task_test.jsonl'
                    else:
                        attacker_tools_path = attacker_tools_override or attack_tool_type

                    log_memory_type = 'new_memory' if read_db else 'no_memory'
                    log_base = f'{log_path}/{defense_type}' if defense_type else f'{log_path}/{log_memory_type}'
                    log_file = f'{log_base}/{attack_type}-{attack_tool_type}'
                    os.makedirs(os.path.dirname(log_file), exist_ok=True)


                    base_cmd = f'''nohup python main_attacker.py --llm_name {llm} --attack_type {attack_type} --use_backend {backend} --attacker_tools_path {attacker_tools_path} --res_file {log_file}{suffix_str}.csv'''

                    if tasks_path_arg:
                        base_cmd += f' --tasks_path {tasks_path_arg}'
                    if task_num:
                        base_cmd += f' --task_num {task_num}'

                    if database:
                        base_cmd += f' --database {database}'
                    if write_db:
                        base_cmd += ' --write_db'
                    if read_db:
                        base_cmd += ' --read_db'
                    if defense_type:
                        base_cmd += f' --defense_type {defense_type}'

                    if injection_method in ['direct_prompt_injection', 'memory_attack', 'observation_prompt_injection', 'clean']:
                        specific_cmd = f' --{injection_method}'
                    elif injection_method == 'mixed_attack':
                        specific_cmd = ' --direct_prompt_injection --observation_prompt_injection'
                    elif injection_method == 'DPI_MP':
                        specific_cmd = ' --direct_prompt_injection'
                    elif injection_method == 'OPI_MP':
                        specific_cmd = ' --observation_prompt_injection'
                    elif injection_method == 'DPI_OPI':
                        specific_cmd = ' --direct_prompt_injection --observation_prompt_injection'
                    else:
                        specific_cmd = ''

                    cmd = f"{base_cmd}{specific_cmd} > {log_file}{suffix_str}.log 2>&1 &"

                    print(f'{log_file}{suffix_str}.log')
                    os.system(cmd)
